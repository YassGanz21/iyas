...
const config = {
    A187: '👾YasBot👾', // TOBAT SU ASU
    instagram: 'https://instagram.com/adamrais.h', // INFO JANGAN DI UBAH
    nomer: 'wa.me/6283879241802', // INFO SU JNGAN DI UBAH
    youtube: 'https://youtube.com/channel/', // KINTIL
    whatsapp: 'https://chat.whatsapp.com/', // BABI
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Selamat tinggal😝 @${num.split('@')[0]} 𝘁𝗼𝗱`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '️❬ ✔ ❭ succes bro🖤',
				error: {
					stick: 'Yaelah gagal ;(, ulangi aja  ^_^',
					Iv: 'maaf link tidak valid☹️'
				},
				only: {
					group: '❬❗❭ GROUP ONLY BRO',
					ownerG: '❬❗❭ OWNER ONLY BRO',
					ownerB: '❬❗❭  OWNER ONLY BRO',
					admin: '❬❗❭ ADMIN ONLY BRO',
					Badmin: '❬❗❭ GABISA,GW BUKAN ADMIN'
				}
			}
            ...
            case 'kangcopet':
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Dih anjg lu kira gw ga capek apa?,,teks lh kepanjangan anjg😤')
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Prefix Berhasil Di ubah : ${prefix}`)
					break 	
				case 'meme': 
...
break
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ BRYAN BOT BROADCAST ❯\n\n${body.slice(4)}`})
						}
					...
					break
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Berhasil Mengganti Icon Grup! ')
                                        break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
       ...
       client.sendMessage(from, 'Tuh udah aku kasih nomor Owner ⚜️Bryan Bot⚜️,Jngn lupa di save  ><',MessageType.text, { quoted: mek} )
           break    
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Mampos ga jadi admin lagi aihihihihi🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Bjir @${mentioned[0].split('@')[0]} Mampos ga jadi admin lagi aihihihihi🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Nah gitu dong dari tadi kan aku jadi ada kerjaan:v🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Nah gitu dong dari tadi kan aku jado ada kerjaan:v @${mentioned[0].split('@')[0]} 𝘁𝗼𝗱🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂?? ??𝗶𝗺𝗶 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
...
conn.sendMessage(id, '3aalaikumsalam, Ketik !help/!menu/ Contoh !help' ,MessageType.text);
}
else if (text == 'sa55_&55-&lam'){
conn.sendMessage(id, 'Waalaikumsalam, Ketik !help/!menu/ Contoh !help' ,MessageType.text);
}
else if (text == 'asalamualai5-+&&kum'){
conn.sendMessage(id, 'Waalaikumsalam, Ketik !help/!memu/ Contoh !help' ,MessageType.text);
}
else if (text == 'Assalamual656&aikum'){
conn.sendMessage(id, 'Waalaikumsalam, Ketik !help/!menu/ Contoh !help' ,MessageType.text);
}
else if (text == 'p'){
conn.sendMessage(id, 'Ya?, Ketik !help/!menu/ Contoh !help' ,MessageType.text);
}
else if (text == 'P'){
conn.sendMessage(id, 'Ya?, Ketik !help/!menu/ Contoh !help' ,MessageType.text);
}
...
conn.sendMessage(id, 'Owner IYAS GANZ wa.me/+6283879241802' ,MessageType.text);
}
else if (text == 'iyas'){
conn.sendMessage(id, 'Aku BOT nya iyas' ,MessageType.text);
}
...
else if (text == 'halo bot'){
conn.sendMessage(id, 'halo juga kamu, baik" aja kan kamu?' ,MessageType.text);
}
else if (text == 'ini bukan bot'){
conn.sendMessage(id, 'GAK TAU SISTEM BOT DIAM AJA BROW' ,MessageType.text);
}
else if (text == 'ini manusia'){
conn.sendMessage(id, 'PAAN SIH GAJE -_' ,MessageType.text);
}
else if (text == 'bang sewa bot brp?'){
conn.sendMessage(id, '20K PERMANEN NGAB' ,MessageType.text);
}
else if (text == 'L4H?'){
conn.sendMessage(id, 'L4H? APA ANJ' ,MessageType.text);
}
else if (text == 'Bang){
conn.sendMessage(id, 'Ya?, Ketik !help/!menu/ Contoh !help' ,MessageType.text);
}
else if (text == 'LOVE YOU'){
conn.sendMessage(id, 'DIH HOMO GBLK' ,MessageType.text);
}
else if (text == 'TOD'){
conn.sendMessage(id, 'PAAN TOD BACOD AE' ,MessageType.text);
}
else if (text == 'makasi'){
